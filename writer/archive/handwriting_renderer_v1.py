...
class HandwritingRenderer:
    ...
    def _get_ink_bounds_x(self, image):
    ...
    def __init__(self, seed=7):
        ...
        self.font_path = "assets/fonts/Caveat-VariableFont_wght.ttf"
    ...
    def _draw_glyph_line(self, image, line, x, baseline_y, target_height, text_color):
        for character in line:
            ...
            # ===================================
            # CONSISTENT NOTEBOOK BASELINE
            # ===================================

            base_body_height = int(target_height * 1.05)

            # Default characters sit ON line
            # y = baseline_y - base_body_height
            y = baseline_y - int(target_height * 1.02)

            # Descenders naturally dip below line
            if character in "gjpqy":
                y += int(target_height * 0.18)

            # Tall ascenders
            elif character in "bdfhklt":
                y -= int(target_height * 0.04)

            # f slightly lower
            if character == "f":
                y += int(target_height * 0.06)

            # Tiny natural handwriting movement
            y += self.random.choice([0, 0, 1, -1])

            # Draw glyph
            image.alpha_composite(tinted, (int(x), int(y)))

            # Natural character spacing
            left_ink, right_ink = self._get_ink_bounds_x(tinted)

            visible_width = right_ink - left_ink

            base_advance = visible_width * 0.85
    ...
    def render_layout_blocks(
        ...
        max_text_width = (
            width
            - margin_left
            - page_template["right_edge"]
        )

        target_glyph_height = min(
            26,
            line_gap - 24
        )
        ...
        line_y = page_template["first_line_y"]
        ...
        for block in blocks:
            ...
            if block_type == "title":
                ...
                # Start body EXACTLY on notebook line
                line_y = page_template["first_line_y"]
